<div class="field {{ $errors->has('fee') ? 'error' : '' }}">
    ...
    <div class="ui right labeled input">
        <input v-model="fields.fee" type="text" name="fee" placeholder="{{ __('app.fee') }}" required autofocus>
...
<template>
    ...
    <div v-for="(amount, i) in fields.payouts.amounts" class="field {{ $errors->has('fee') ? 'error' : '' }}">
        ...
        <div class="ui left action right labeled input">
            <span v-if="i == fields.payouts.amounts.length-1" class="ui icon button" @click="removePayout">
                <i class="minus icon"></i>
            </span>
            <span v-else class="ui disabled icon button">
                <i class="icon"></i>
            </span>
            <select v-model="fields.payouts.types[i]" name="payouts_types[]" class="ui competition-payout-type dropdown">
                <option value="flat">{{ __('app.payout_type_flat') }}</option>
                <option value="percentage">{{ __('app.payout_type_percentage') }}</option>
            </select>
            <input v-model="fields.payouts.amounts[i]" type="text" name="payouts_amounts[]" placeholder="{{ __('app.fee') }}" required autofocus>
            <div v-if="fields.payouts.types[i]=='flat'" class="ui basic label">{{ config('settings.currency') }}</div>
